...
class Bird {
	...
	// Link cell indeces
	unsigned i, j;
	// Bool if the number of cells in the length is exact
	bool exact_cells;
...
double Bird::average_angle(const std::list<Bird>& birds, const std::vector< std::vector<unsigned> >& neighbours,
	...
	unsigned n, N_neigh = neighbours.size(), N_cells;
...
int main() {
	...
	std::ofstream fbirds("birds_" + std::to_string(N_birds) + "_N_" + std::to_string(N)
			+ "_L_" + std::to_string(L) + "_movement_d_link.txt");
	...
	for (i = 1; i < N; i++) {
		...
		for (it = bird_list.begin(); it != bird_list.end(); it++)
			fbirds << i*N_steps << '\t' << it->get_x() << '\t' << it->get_y() << '\t' << it->get_angle() << '\n';
		fbirds << '\n';
	}
