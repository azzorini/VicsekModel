...
class Bird {
	...
	// Link cell indeces
	unsigned i, j;
	// Variable that tell us if the number of cells exactly match the length
	bool exact_cells;
...
: v(vel), L(length), exact_cells(!(std::ceil(length) > length)), gen(g), ran_theta(-eta/2, eta/2), ran_u(0, 1),
...
int main() {
	...
	unsigned i, j;
	std::ofstream fout("VAvsT_N_" + std::to_string(N_birds) + "_L_" + std::to_string(L) + ".txt");
	...
	fout << 0 << '\t' << compute_va(bird_list, N_birds) << '\n';
	for (i = 1; i < N; i++) {
		for (j = 0; j < N_steps; j++) {
		...
		fout << i*N_steps << '\t' << compute_va(bird_list, N_birds) << '\n';
	}
