...
public:
	...
	double average_angle(const std::vector< std::vector<unsigned> >&,
	...
	virtual void update(const std::vector< std::vector<unsigned> >&,
			std::vector< std::list<Bird*> >&, unsigned);

	// Same as update but instead of updating it
	// it saves everything in a new variable
	virtual void update_to_new(const std::vector< std::vector<unsigned> >&,
...
double Bird::average_angle(const std::vector< std::vector<unsigned> >& neighbours,
...
void Bird::update(const std::vector< std::vector<unsigned> >& neighbours,
			std::vector< std::list<Bird*> >& link_list, unsigned N_cells) {
	unsigned ind = pos_by_indeces(i, j, N_cells);
	bool changed = false;
	double avg_angle = average_angle(neighbours, link_list, ind);
	unsigned aux;
...
void Bird::update_to_new(const std::vector< std::vector<unsigned> >& neighbours,
	...
	// We update the angle
	theta_new = average_angle(neighbours, link_list, pos_by_indeces(i, j, N_cells)) + ran_theta(*gen);
}
...
int main() {
	...
	const unsigned N_measures = 5000;
	...
	double eta = 5.;
	std::ofstream fout("VAvsETA_N_" + std::to_string(N_birds) + "_L_" + std::to_string(L) + "_p.txt");
	...
	for (i = 0; i < N_relax_ini; i++) {
			...
			for (it = bird_list.begin(); it != bird_list.end(); it++) it->update_to_new(neighbours, link_list, N_cells);
